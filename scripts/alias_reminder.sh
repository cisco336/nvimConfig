# Show all aliases from ~/.zshrc
show_zshrc_aliases() {
	print_header "📝 ALIASES FROM ~/.zshrc"
	if [[ -f ~/.zshrc ]]; then
		grep -E "^alias " ~/.zshrc | sed -E "s/^alias ([^=]+)='([^']+)'/  \1 → \2/; s/^alias ([^=]+)=(\"[^\"]+\")/  \1 → \2/" | while read -r line; do
			printf "%s\n" "$line"
		done
	else
		print_color $RED "No ~/.zshrc file found."
	fi
}
#!/bin/bash
...
show_custom_aliases() {
	...
	printf "  %-12s → %-30s %s\n" "aliases" "~/alias_reminder.sh" "Show alias reminder"
	printf "  %-12s → %-30s %s\n" "cd" "z" "Smart directory jump"

	echo ""
	echo "Git (Custom):"
	printf "  %-12s → %-30s %s\n" "fetch" "git fetch --all" "Fetch all remotes"
	printf "  %-12s → %-30s %s\n" "ph" "git push" "Short push (gp)"
	printf "  %-12s → %-30s %s\n" "pl" "git pull" "Short pull (gl)"
	printf "  %-12s → %-30s %s\n" "commit" "git cz c" "Commitizen commit"
	printf "  %-12s → %-30s %s\n" "sts" "git status" "Shortest status"
	printf "  %-12s → %-30s %s\n" "greset" "git reset --hard HEAD" "Hard reset"
	printf "  %-12s → %-30s %s\n" "pulldevelop" "git checkout develop && git pull" "Pull develop branch"
	printf "  %-12s → %-30s %s\n" "pullmain" "git checkout main && git pull" "Pull main branch"
	printf "  %-12s → %-30s %s\n" "lg" "lazygit" "LazyGit TUI"

	echo ""
	echo "Development:"
	printf "  %-12s → %-30s %s\n" "rundev" "npm run dev" "Start dev server"
	printf "  %-12s → %-30s %s\n" "coverageweb" "npm run test:unit:coverage" "Show coverage in browser"

	echo ""
	echo "Utility:"
	printf "  %-12s → %-30s %s\n" "fonts" "atsutil fonts -list" "List all system fonts"
	printf "  %-12s → %-30s %s\n" "chromenocors" "open -n -a ... --disable-web-security" "Chrome no CORS"
	printf "  %-12s → %-30s %s\n" "myip" "curl -s ifconfig.me; echo" "Show public IP"
	printf "  %-12s → %-30s %s\n" "geoip" "curl -s ipinfo.io" "Show geo IP info"
	printf "  %-12s → %-30s %s\n" "qr" "curl -F '-=<-' qrenco.de" "Generate© QR code"
	printf "  %-12s → %-30s %s\n" "quote" "curl -s ...quotable.io... | jq ..." "Random quote"
	printf "  %-12s → %-30s %s\n" "advice" "curl -s ...adviceslip.com... | jq ..." "Random advice"

	echo ""
	echo "FZF Enhanced:"
	printf "  %-12s → %-30s %s\n" "branch" "git branch | fzf | xargs git checkout" "Interactive branch switch"
	printf "  %-12s → %-30s %s\n" "tag" "git tag | fzf | xargs git checkout" "Interactive tag switch"
}
...
main() {
	...
	--help | -h | help)
		...
		echo "  $(basename $0) zshrc     Show aliases from ~/.zshrc"
		;;
	*)
		print_color $CYAN "Oh My Zsh Alias Quick Reference"
		print_color $CYAN "Run with: aliases [git|npm|util|custom|workflow|tips|zshrc]"
